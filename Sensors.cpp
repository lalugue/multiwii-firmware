...
uint8_t Baro_update() {                   // first UT conversion is started in init procedure
  ...
  bmp280_ctx.deadline = currentTime+5000; // 1.5ms margin according to the spec (4.5ms T convetion time)
  if (bmp280_ctx.state == 0) {
    //i2c_BMP085_UT_Read(); 
    //i2c_BMP085_UP_Start();
    bmp280_ctx.altitude = bmp.readAltitude(1013.00)/100; 
    bmp280_ctx.temp = bmp.readTemperature(); 
    bmp280_ctx.pressure = bmp.readPressure(); 
    bmp280_ctx.state = 1; 
    Baro_Common();
    //bmp280_ctx.deadline += 5000;   // 6000+21000=27000 1.5ms margin according to the spec (25.5ms P convetion time with OSS=3)
    return 1;
  } else {
    //i2c_BMP085_UP_Read(); 
    //i2c_BMP085_UT_Start(); 
    //i2c_BMP085_Calculate();
    bmp280_ctx.altitude = bmp.readAltitude(1013.25)/100; 
    bmp280_ctx.temp = bmp.readTemperature(); 
    bmp280_ctx.pressure = bmp.readPressure(); 
    baroPressure = bmp280_ctx.pressure;
    bmp280_ctx.state = 0; 
    return 2;
  }
}
